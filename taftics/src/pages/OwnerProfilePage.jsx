...
export default function OwnerProfilePage({ user, setUser }) {
  ...
  useEffect(() => {
    if (
      !user ||
      (!user.ownedEstablishmentId && !user.ownedEstablishmentId?.$oid)
    ) {
      setLoading(false);
      return;
    }
    ...
    const targetId =
      user.ownedEstablishmentId?.$oid || user.ownedEstablishmentId;

    Promise.all([
      fetch(
        `${import.meta.env.VITE_API_URL}/api/establishments/${targetId}`,
      ).then((r) => r.json()),
      fetch(
        `${import.meta.env.VITE_API_URL}/api/establishments/${targetId}/reviews`,
      ).then((r) => r.json()),
    ])
  ...
  const formattedReviews = reviews
    .map((rev) => {
      ...
      return {
        ...
        body: rev.body,
        user: {
          name: rev.userId?.name || rev.userId?.username || "Unknown User",
        },
    ...
    .sort((a, b) =>
      a.hasOwnerReply === b.hasOwnerReply ? 0 : a.hasOwnerReply ? 1 : -1,
    );
      ...
      <div className="container mt-4">
        {loading ? (
          <div className="text-center p-5">
            <span className="text-muted fw-bold">Loading dashboard...</span>
          </div>
        ) : !establishment ? (
          <div className="text-center p-5">
            <span className="text-danger fw-bold">
              Warning: User is not linked to an active Business Entity.
            </span>
          </div>
        ) : (
            ...
            <div className="col-lg-3">
              <div className="custom-card p-4 sidebar-sticky text-center">
                ...
                <p className="small text-muted mb-4 opacity-75">
                  Managing:{" "}
                  <strong className="text-dlsu-dark">
                    {establishment.name}
                  </strong>
                </p>
                ...
                <Link
                  to={`/establishment/${establishment._id || establishment.id}`}
                  className="btn btn-dark w-100 d-flex justify-content-center align-items-center gap-2 text-decoration-none"
                >
            ...
            <div className="col-lg-9">
              ...
              <div className="d-flex flex-wrap gap-2 mb-4 p-2 bg-white rounded-4 shadow-sm border">
                ...
                <TabButton
                  id="inbox"
                  icon={Star}
                  label={`Reviews Inbox (${formattedReviews.filter((r) => !r.hasOwnerReply).length})`}
                />
              </div>
                ...
                <div className="row g-3">
                  <div className="col-md-4">
                    <div className="custom-card p-4 text-center h-100 d-flex flex-column justify-content-center">
                      <h1
                        className="fw-bold text-dlsu-dark mb-1"
                        style={{ fontSize: "3rem" }}
                      >
                        {reviews.length > 0
                          ? (
                  ...
                  <div className="col-md-4">
                    <div className="custom-card p-4 text-center h-100 d-flex flex-column justify-content-center bg-dlsu-light border border-success">
                      ...
                      >
                        {
                          formattedReviews.filter((r) => !r.hasOwnerReply)
                            .length
                        }
                      </h1>
                  ...
                  <form>
                    <div className="row g-3 mb-4">
                      ...
                      <div className="col-12">
                        ...
                        {isEditingStore ? (
                          ...
                          <div className="form-control bg-light fw-bold text-muted border-0 py-2">
                            {establishment.name}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="col-12 mt-3">
                        ...
                        >
                          <img
                            src={
                              establishment.image ||
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(establishment.name)}&background=00441b&color=fff&size=512&bold=true`
                            }
                            alt="Store cover"
                    ...
                    {isEditingStore ? (
                      <div className="d-flex gap-3 pt-3 border-top mt-4">
                        <button
                          type="button"
                          className="btn btn-dlsu-dark fw-bold px-4 rounded-pill"
                      ...
                      <div className="d-flex flex-wrap gap-2 pt-3 border-top mt-4">
                        <button
                          type="button"
                          className="btn btn-dlsu-dark fw-bold px-4 rounded-pill flex-grow-1"
                ...
                <div className="d-flex flex-column gap-3">
                  {formattedReviews.filter((r) => !r.hasOwnerReply).length >
                    ...
                    <div className="alert alert-warning border border-warning rounded-4 small mb-1 shadow-sm d-flex align-items-center gap-2">
                      <strong className="fs-6">Action required:</strong> You
                      have{" "}
                      {formattedReviews.filter((r) => !r.hasOwnerReply).length}{" "}
                      unanswered reviews. Engaging with customers improves your
                      rating!
                    </div>
                  )}
                  ...
                  {formattedReviews.map((rev) => (
                    <div
                      key={rev.id}
                      className="custom-card p-4 border shadow-sm"
                    >
                      <div className="d-flex justify-content-between mb-3 align-items-start">
                        <div className="d-flex gap-3 align-items-center">
                          <div
                            className="bg-secondary rounded-circle text-white d-flex align-items-center justify-content-center fw-bold"
                            style={{
                              width: 45,
                              height: 45,
                              fontSize: "1.2rem",
